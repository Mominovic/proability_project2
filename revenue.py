...
revenue.py
==========
Calculates revenue, profit, and optimal production quantity
for the HelioFold Powerbank at each surveyed price point.
...
def compute_revenue_table(cost=PRODUCTION_COST):
    """
    Returns a list of dicts with revenue/profit at each price point.
    Uses actual survey demand (not regression-predicted) for accuracy.
    """
...
def find_optimal(cost=PRODUCTION_COST):
    """Finds the price-quantity pair that maximises total profit."""
...
def sensitivity_analysis():
    """
    Runs optimal production analysis for a range of production costs.
    Mirrors the textbook table structure.
    """
